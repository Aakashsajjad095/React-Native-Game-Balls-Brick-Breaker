...
//   const tickSound = new Sound('tickMessage.mp3', Sound.MAIN_BUNDLE, (error) => {
//     if (error) {
...
function playsound() {
  ...
  const tickSound = new Sound('hitball.wav', Sound.MAIN_BUNDLE, (error) => {
    if (error) {
      console.error('Failed to load sound playsound ', error);
    } else {
      console.log('Sound loaded successfully');
...
function playPowerUpSound() {
  ...
  const powerUpSound = new Sound('powerUpSound.wav', Sound.MAIN_BUNDLE, (error) => {
    if (error) {
      console.error('Failed to load power-up sound playPowerUpSound', error);
    } else {
      console.log('Power-up sound loaded successfully');
      // Play the power-up sound with an onEnd callback
      powerUpSound.play((success) => {
        if (success) {
          console.log('Power-up sound finished playing');
        } else {
          console.error('Playback failed due to audio decoding errors');
        }
      });
...
// Call playPowerUpSound() when a power-up box is collected.


function playLevelCompletionSound() {
  Sound.setCategory('Playback');

  const levelCompletionSound = new Sound('levelCompletionSound.wav', Sound.MAIN_BUNDLE, (error) => {
    if (error) {
      console.error('Failed to load level completion sound playLevelCompletionSound', error);
    } else {
      console.log('Level completion sound loaded successfully');
      // Play the level completion sound with an onEnd callback
      levelCompletionSound.play((success) => {
        if (success) {
          console.log('Level completion sound finished playing');
        } else {
          console.error('Playback failed due to audio decoding errors');
        }
      });
...
// Call playLevelCompletionSound() when a level is completed.
function playGameOverSound() {
  Sound.setCategory('Playback');

  const gameOverSound = new Sound('gameOverSound.wav', Sound.MAIN_BUNDLE, (error) => {
    if (error) {
      console.error('Failed to load game over sound playGameOverSound', error);
    } else {
      console.log('Game over sound loaded successfully');
      // Play the game over sound with an onEnd callback
      gameOverSound.play((success) => {
        if (success) {
          console.log('Game over sound finished playing');
        } else {
          console.error('Playback failed due to audio decoding errors');
        }
      });
...
function boxBreakSound() {
  Sound.setCategory('Playback');

  const gameOverSound = new Sound('boxBreak.wav', Sound.MAIN_BUNDLE, (error) => {
    if (error) {
      console.error('Failed to load game over sound boxBreakSound', error);
    } else {
      console.log('Game over sound loaded successfully');
      // Play the game over sound with an onEnd callback
      gameOverSound.play((success) => {
...
export function moveToNextLevel(entities, dispatch) {
  ...
  dead_boxes.forEach(deadBoxeId => {
    delete entities[deadBoxeId];
  });
  // recalculate the boxes that are left
  boxes = Object.keys(entities).filter(key => key.startsWith('box'));
  ...
  if (scorebar.state === Config.STOPPED) {
    touches
      ...
      .forEach(t => {
        ...
        drag_vector.delta = {x: 0, y: 0};
        drag_vector.final = {x: 0, y: 0};
        const aim_line = Utils.newPosition(
          ball.position.x + Config.RADIUS / 2,
          ball.position.y + Config.RADIUS / 2,
        );
        entities.aimline = {
...
const SpeedUp = (entities, {touches, time}) => {
  ...
  touches
    ...
    .forEach(t => {
      if (speedbutton.available) {
        const top = Utils.rowToTopPosition(speedbutton.row);
        const left = Utils.colToLeftPosition(speedbutton.column);
        const eventX = t.event.pageX;
        const eventY = t.event.pageY;
        if (
          eventX > left &&
          eventX < left + Config.BOX_TILE_SIZE &&
          eventY > top &&
